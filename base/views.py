...
def index(request):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            total_amount_sum = Bill.objects.aggregate(total_amount_sum=Sum('total_amount'))['total_amount_sum']
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                # data.save()
                    ...
                    bill = Bill(name=sequence,total_amount=total_amount,created_by=request.user)
                    bill.save()
                    for product_item in product_list:
                        product_id = Product.objects.get(id=product_item['id'])
                        get_product_name = "product_name_"+str(product_item['id'])
                        ...
                        bill_item = BillItem(bill_id=bill,product_id=product_id,product_name=product_name,rate=rate,qty=qty,total=total)
                        bill_item.save()
